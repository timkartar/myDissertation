...
Starting from the mean field variational inference result of a CRF as given in \red{(Kr\"{a}henb\"{u}hl, 2012)} we have,
...
Focusing on the inner most summation of second term, since each component of $H_i$ and $H_j$ is independent (by eq. 10 of \red{Gao et al.}), we can write $Q(H_j) = \Pi_{k=1}^N Q_{jk}(H_{jk})$ and assume that each $Q_{jk}$ is individually normalized. The summation then becomes
